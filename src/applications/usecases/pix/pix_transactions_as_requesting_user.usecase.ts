...
export class PixTransactionAsRequestingUserUsecase implements IPixTransactionAsRequestingUserUsecase {
  constructor(
  ...
  async get(user: Partial<User>): Promise<Either<BaseError, Pix[] | undefined>> {
    ...
    const pixTransactions = await this._pixRepository.getTransactionsFromRequestingUserId(userExists.value.id);
